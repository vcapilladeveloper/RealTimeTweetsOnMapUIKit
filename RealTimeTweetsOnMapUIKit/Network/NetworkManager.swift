...
open class NetworkManager {
    ...
    public static func requestData<T, E: Endpoint>(_ endpoint: E, completion: @escaping (NetworkResult<T>) -> Void) {
        ...
        dataTask = defaultSession.dataTask(with: urlRequest, completionHandler: { data, _, error in
            if let error = error {
...
private extension NetworkManager {
    ...
    static func makeRequest<E: Endpoint>(_ endpoint: E) -> URLRequest {
        ...
        // URL
        guard let url = URL(string: endpoint.baseURLString + endpoint.path) else {
            fatalError("Endpoint URL is invalid")
        }
